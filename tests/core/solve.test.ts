...
describe('12.9 + 6061-T6 觸發警示（派工單 §5 驗收項）', () => {
  ...
  it('產生 parent-thread-weaker 警示，內容含降級建議與扭矩規範', () => {
    ...
    const w = r.warnings.find((x) => x.message.includes('崩牙會先於斷螺絲'));
    expect(w).toBeDefined();
    expect(w!.level).toBe('warn');
    expect(w!.message).toMatch(/A2-70/);
    expect(w!.message).toMatch(/2\.5d/);
    expect(w!.message).toMatch(/扭矩規範/);
  });

  it('A2-70 + S45C 不觸發該警示', () => {
    const r = solve(inputs(), D);
    expect(r.warnings.some((x) => x.message.includes('崩牙會先於斷螺絲'))).toBe(false);
  });
});
...
describe('SUS304 母材觸發 anti-seize 警示', () => {
  ...
  it('兩個等級都觸發，且明示不要調高嵌入', () => {
    for (const grade of ['A2-70', '12.9'] as const) {
      ...
      expect(w!.message).toMatch(/嵌入越長咬死機率越高/);
    }
  });
});
...
describe('c_min > c_max 回 infeasible 並帶可讀理由（派工單 §5 驗收項）', () => {
  ...
  it('理由必須說明怎麼修，不只說不行', () => {
    ...
    const e = r.warnings.find((w) => w.level === 'error')!;
    expect(e.message).toMatch(/低頭型|去掉墊圈|加厚/);
  });
});

describe('牙深不足（模式 B 的主要輸出）', () => {
  const D = db();

  it('現有孔牙深不夠時，點名牙深並算出最小需求 H', () => {
    // M6 + 12.9 + 6061-T6：kMin 2.0 → E_min 12；H=10 → hEff 7 → E_max 5.5
    const r = solve(
      inputs({ grade: '12.9', parentMaterial: '6061-T6', plateMaterial: '6061-T6', tapDepth: 10 }),
      D,
    );
    expect(r.feasible).toBe(false);
    const e = r.warnings.find((w) => w.level === 'error')!;
    expect(e.message).toMatch(/有效牙深不足/);
    expect(e.message).toMatch(/H ≥ 16\.5 mm/); // 12 + 1.5 + 3
  });
});
...
describe('標準長度無交集：是 warn 不是 error', () => {
  ...
  it('寫出算出來的區間讓人知道是差多少，而不是工具壞了', () => {
    ...
    const w = r.warnings.find((x) => x.message.includes('可行長度區間'));
    expect(w).toBeDefined();
...
describe('上件材質：未指定時以母材代入並發 warn', () => {
  ...
  it('未指定 → warn，且說明受力面在上件', () => {
    ...
    const w = r.warnings.find((x) => x.message.includes('上件材質未指定'));
    expect(w).toBeDefined();
    expect(w!.level).toBe('warn');
    expect(w!.message).toMatch(/沉頭底面是受力面/);
    expect(r.derivation.remainingWallMaterialAssumed).toBe(true);
  });

  it('明確指定 → 無此 warn', () => {
    const r = solve(inputs({ plateMaterial: 'S45C' }), D);
    expect(r.warnings.some((x) => x.message.includes('上件材質未指定'))).toBe(false);
    expect(r.derivation.remainingWallMaterialAssumed).toBe(false);
  });

  it('鋁蓋鎖鋼座：上件材質改變殘留肉厚下限（2.0 → 3.0）', () => {
...
describe('牙深來源與資料版本', () => {
  ...
  it('未查證的值會逐項列出，不安靜通過', () => {
    ...
    const w = r.warnings.find((x) => x.message.includes('尚未查證'));
    expect(w).toBeDefined();
    expect(w!.level).toBe('warn');
  });
...
describe('推導值完整可追溯（展開層的資料來源）', () => {
  ...
  it('derivation 帶齊 k_min/k_cap 出處、δ、H_eff、runout', () => {
    const d = solve(inputs(), D).derivation;
    expect(d.kMin).toBe(1);
    expect(d.kCap).toBe(1.5);
    expect(d.kRationale).toMatch(/螺絲斷裂/);
    expect(d.delta).toBe(0.5);
    expect(d.hEff).toBe(11);
    expect(d.imperfectPitches).toBe(3);
...
describe('基準案例：M6 / A2-70 / S45C / t=12 / ISO7089', () => {
  ...
  it('幾何與手算一致', () => {
    ...
    expect(r.geometry.drillDepth).toBe(18);
  });
  ...
  it('界線與手算一致', () => {
    ...
    expect(r.derivation.eMax).toBe(9); // min(1.5×6, 11−1.5)
